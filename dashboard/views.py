...
def buscar_dados_completos(sp=None):
    dados = {
        ...
        'sistema': {
            ...
            'discos': [], 'gpu': None,
            'rede': {'ping': 0, 'nome': '...', 'download_mbps': 0, 'upload_mbps': 0}
        },
        'spotify': None,
        'erro': None
    }
    ...
    try:
            ...
            try:
                ...
                if current and current.get('item'):
                    ...
                    dados['spotify'] = {
                        ...
                        'link_externo': track['external_urls']['spotify'],
        ...
        try:
            ...
            # Dados de Carga e Memória
            util = pynvml.nvmlDeviceGetUtilizationRates(handle)
            mem_info = pynvml.nvmlDeviceGetMemoryInfo(handle)
            
            # Cálculo de Porcentagem da VRAM
            vram_percent = 0
            if mem_info.total > 0:
        ...
        if 'Ethernet' in stats and stats['Ethernet'].isup: nome_rede = "Rede Cabeada"
        ...
        io_1 = psutil.net_io_counters(); time.sleep(0.1); io_2 = psutil.net_io_counters()
        mbps_down = ((io_2.bytes_recv - io_1.bytes_recv) * 8 * 10) / 1_000_000
        ...
        try:
            ...
            response_crypto = requests.get(url_crypto, timeout=2)
...
def home(request):
    ...
    info = buscar_dados_completos(sp)
    ...
    try:
        if comando == 'next': sp.next_track()
            ...
            if current and current.get('item'):
                ...
                if ja_curtiu: sp.current_user_saved_tracks_delete(tracks=[track_id])
                else: sp.current_user_saved_tracks_add(tracks=[track_id])
        elif comando == 'vol_up': 
...
def callback(request):
    code = request.GET.get('code')
    if code: sp_oauth.get_access_token(code)
    return redirect('home')

def adicionar_nota(request):
    if request.method == "POST":
        texto_nota = request.POST.get('texto')
        if texto_nota: Anotacao.objects.create(texto=texto_nota)
    notas = Anotacao.objects.order_by('-criado_em')
